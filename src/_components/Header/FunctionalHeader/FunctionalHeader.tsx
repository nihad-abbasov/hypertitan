...
const FunctionalHeader: React.FC = () => {
  ...
  return (
    // stickyClassName
    <div className={`${stickyClassName} functioanlHeader`}>
      <div className="container functioanlHeader_wrapper">
        <Link href="/" passHref>
          <Image
            priority
            width={0}
        ...
        <form className="form" name="searchForm">
          <input
            className="text-gray-900 placeholder-slate-700 dark:text-slate-100 dark:placeholder-slate-200"
            type="search"
            name="searchbar"
            placeholder="Axtar..."
            value={searchTerm}
          ...
          {isDropdownVisible && searchTerm.length > 1 && (
            <div className="search-dropdown">
              {searchResults.length > 0 ? (
                ...
                <div className="no-results">Məhsul tapılmadı</div>
              )}
            </div>
          )}
          <button
        ...
        <div className="functional-icons">
          ...
          {/* <Link passHref href="/compare" className="icon-wrapper">
            <svg
              stroke="currentColor"
              fill="currentColor"
              strokeWidth="0"
          ...
          {isAuth && (
            <Link passHref href="/favorites" className="icon-wrapper">
              <svg
                stroke="currentColor"
                fill="currentColor"
                strokeWidth="0"
              ...
              <span className="badge">{favoritesAmount}</span>
            </Link>
          )}

          <button onClick={handleMiniCartToggle} className="icon-wrapper cart">
            <div
              className={`basket-wrapper ${isBasketPopping && "pop-animation"}`}
            >
              <svg
                stroke="currentColor"
                fill="currentColor"
              ...
              <span className="badge basket">{uniqueTotalProductAmount}</span>
            </div>
          ...
          {isAuth ? (
            <div className="icon-wrapper linkToAccount">
